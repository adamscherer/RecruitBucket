...
public class PagingAndSorting implements Pageable {
	...
	public PagingAndSorting(String pageParameter, String limitParameter, String orderParameter, PageRequest pageRequest) {
	...
	public PagingAndSorting addSortParameters(UriComponentsBuilder urib) {
		...
		if (null != sort) {
			...
			while (iter.hasNext()) {
				...
				try {
					urib.queryParam(URLEncoder.encode(order.getProperty() + ".dir", "ISO-8859-1"), order.getDirection().toString().toLowerCase());
